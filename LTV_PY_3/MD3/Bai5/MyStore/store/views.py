...
def send_registration_email(user_email, username):
    ...
    message = 'Hãy trải nghiệm việc mua sắm online các thiết bị Đồ dùng nhà bếp và Thiết bị gia đình tại My Store.'
    from_email = settings.EMAIL_HOST_USER

    # Nội dung HTML của email
    html_content = f'''
        <h2 style="color:blue"><i>Kính chào {username},</i></h2>
        <p>Chào mừng quý khách đến với <strong>My Store</strong> website.</p>
        <h4 style="color:red">{message}</h4>
    '''

    # Gửi email với nội dung HTML
    msg = EmailMultiAlternatives(subject, message, from_email, [user_email])
